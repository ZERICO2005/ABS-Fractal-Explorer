...
enum RenderType {SingleThread,MultiThread,OpenCL};
enum RenderType renderType = OpenCL;
	...
	while (1) {
		...
		if (timerReady(&keyInputTimer)) {
			...
			if ((keyPressed(SDL_SCANCODE_RETURN) || (keyPressed(SDL_SCANCODE_Z) && numberAcc != 0)) && numberInputSelect == 2) { //Formula
				if (input.type == 2) { numberAcc %= 256; } else if (input.type == 3) { numberAcc %= 16384; } else if (input.type == 4) { numberAcc %= 131072; } else if (input.type == 5) { numberAcc %= 1048576; }
			...
			if (keyPressed(SDL_SCANCODE_M)) { renderType = OpenCL; update = 1; }
...
void renderFractal(frac* param) {
	if (renderType == SingleThread) {
		//renderFormula_SingleThread(param,VRAM_Render, RESX_Render, RESY_Render);
		renderFormula_MultiThread(param,VRAM_Render, RESX_Render, RESY_Render, (threadCount == 1) ? 1 : (threadCount / 2));
	} else if (renderType == MultiThread) {
		renderFormula_MultiThread(param,VRAM_Render, RESX_Render, RESY_Render, threadCount * 4);
	} else if (renderType == OpenCL) {
		renderFormula_OpenCL(param,VRAM_Render, RESX_Render, RESY_Render);
	}
