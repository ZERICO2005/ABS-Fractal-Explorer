...
/* Function Definition AVX FP64 */
	...
	void CPU_Interior_Coloring_AVX_FP64(
	...
	void CPU_Exterior_Coloring_AVX_FP64(
		fp64& outR, fp64& outG, fp64& outB, fp64& outA,
		const PreCalc_Param<fp64, fp64>& param,
	...
	#define Block_BeginLoop_AVX_FP64(); \
		...
		const __m256d alphaDiv       = _mm256_set1_pd(param.alphaDiv      );\
		/* Color Values */\
		...
		for (; y < param.Cord_ResY; y += param.sample) {\
			for (; x < param.Cord_ResX; x += (int32_t)SIMD_Spacing * param.sample) {\
				...
				/* Store the output color values */\
					__m256d outR = _mm256_setzero_pd();\
					__m256d outG = _mm256_setzero_pd();\
					__m256d outB = _mm256_setzero_pd();\
					__m256d outA = _mm256_setzero_pd();\
	...
	#define Block_EndLoop_AVX_FP64(); \
				...
				/* Divide R, G, and B by Alpha */\
					outR = _mm256_div_pd(outR, outA);\
					outG = _mm256_div_pd(outG, outA);\
					outB = _mm256_div_pd(outB, outA);\
				/* If division by zero occured, set values to 0 */\
					const __m256d div_zero_mask = _mm256_cmp_pd(outA, _mm256_setzero_pd(), _CMP_EQ_OQ);\
					outR = _mm256_andnot_pd(div_zero_mask, outR);\
					outG = _mm256_andnot_pd(div_zero_mask, outG);\
					outB = _mm256_andnot_pd(div_zero_mask, outB);\
				\
				/* Normilizes alpha to 0.0 - 1.0 */\
					outA = _mm256_div_pd(outA, alphaDiv);\
				/* Normilizes alpha to 0.0 - 1.0 */\
					const __m256d color_mult = _mm256_set1_pd(255.0);\
					outR = _mm256_mul_pd(outR, color_mult);\
					outG = _mm256_mul_pd(outG, color_mult);\
					outB = _mm256_mul_pd(outB, color_mult);\
					outA = _mm256_mul_pd(outA, color_mult);\
				\
...
/* Function Definition AVX FP32 */
	...
	void CPU_Interior_Coloring_AVX_FP32(
	...
	void CPU_Exterior_Coloring_AVX_FP32(
		fp32& outR, fp32& outG, fp32& outB, fp32& outA,
		const PreCalc_Param<fp32,fp32>& param,
	...
	#define Block_BeginLoop_AVX_FP32(); \
		...
		/* Constant used to divide Alpha (sample * sample) */\
		const __m256 alphaDiv       = _mm256_set1_ps(param.alphaDiv      );\
		/* Color Values */\
		...
		for (; y < param.Cord_ResY; y += param.sample) {\
			for (; x < param.Cord_ResX; x += (int32_t)SIMD_Spacing * param.sample) {\
				...
				/* Store the output color values */\
					__m256 outR = _mm256_setzero_ps();\
					__m256 outG = _mm256_setzero_ps();\
					__m256 outB = _mm256_setzero_ps();\
					__m256 outA = _mm256_setzero_ps();\
				/* Calculates 4 pixels (with super sampling) at a time */\
				for (int32_t v = 0; v < param.sample; v++) {\
					/* Calculates y cordinate-value */\
		...
		#define Block_EndLoop_AVX_FP32(); \
				...
				\
				/* Divide R, G, and B by Alpha */\
					outR = _mm256_div_ps(outR, outA);\
					outG = _mm256_div_ps(outG, outA);\
					outB = _mm256_div_ps(outB, outA);\
				/* If division by zero occured, set values to 0 */\
					const __m256 div_zero_mask = _mm256_cmp_ps(outA, _mm256_setzero_ps(), _CMP_EQ_OQ);\
					outR = _mm256_andnot_ps(div_zero_mask, outR);\
					outG = _mm256_andnot_ps(div_zero_mask, outG);\
					outB = _mm256_andnot_ps(div_zero_mask, outB);\
				\
				/* Normilizes alpha to 0.0 - 1.0 */\
					outA = _mm256_div_ps(outA, alphaDiv);\
				/* Normilizes alpha to 0.0 - 1.0 */\
					const __m256 color_mult = _mm256_set1_ps(255.0);\
					outR = _mm256_mul_ps(outR, color_mult);\
					outG = _mm256_mul_ps(outG, color_mult);\
					outB = _mm256_mul_ps(outB, color_mult);\
					outA = _mm256_mul_ps(outA, color_mult);\
				\
				for (size_t i = 0; i < valuesToWrite; i++) {\
					/* Writes in the equivilant of SDL_PIXELFORMAT_ABGR8888 */\
