...
/* Function Definition SSE2 FP64 */
	...
	#define color_pow_2_SSE2_FP64(x) (x) * (x)
	...
	static void CPU_Interior_Coloring_SSE2_FP64(
		...
		outputColor[index + 0] += color_pow_2_SSE2_FP64(param.Interior_R_Amp_mult_Interior_Alpha * (0.5 - 0.5 * cos(log(low) * param.Interior_R_Freq + param.Interior_R_Phase_mult_TAU)));
		outputColor[index + 1] += color_pow_2_SSE2_FP64(param.Interior_G_Amp_mult_Interior_Alpha * (0.5 - 0.5 * cos(log(low) * param.Interior_G_Freq + param.Interior_G_Phase_mult_TAU)));
		outputColor[index + 2] += color_pow_2_SSE2_FP64(param.Interior_B_Amp_mult_Interior_Alpha * (0.5 - 0.5 * cos(log(low) * param.Interior_B_Freq + param.Interior_B_Phase_mult_TAU)));
		outputColor[index + 3] += color_pow_2_SSE2_FP64(param.Interior_Alpha);
	}
	static void CPU_Exterior_Coloring_SSE2_FP64(
		...
		outputColor[index + 0] += color_pow_2_SSE2_FP64(param.Exterior_R_Amp_mult_Exterior_Alpha * (0.5 - 0.5 * cos(param.Exterior_R_Freq_mult_TAU * smooth + param.Exterior_R_Phase_mult_TAU)));
		outputColor[index + 1] += color_pow_2_SSE2_FP64(param.Exterior_G_Amp_mult_Exterior_Alpha * (0.5 - 0.5 * cos(param.Exterior_G_Freq_mult_TAU * smooth + param.Exterior_G_Phase_mult_TAU)));
		outputColor[index + 2] += color_pow_2_SSE2_FP64(param.Exterior_B_Amp_mult_Exterior_Alpha * (0.5 - 0.5 * cos(param.Exterior_B_Freq_mult_TAU * smooth + param.Exterior_B_Phase_mult_TAU)));
		outputColor[index + 3] += color_pow_2_SSE2_FP64(param.Exterior_Alpha);
	}
...
/* Function Definition SSE2 FP32 */
	...
	#define color_pow_2_SSE2_FP32(x) (x) * (x)
	...
	static void CPU_Interior_Coloring_SSE2_FP32(
		...
		outputColor[index + 0] += color_pow_2_SSE2_FP32(param.Interior_R_Amp_mult_Interior_Alpha * (0.5f - 0.5f * cosf(logf(low) * param.Interior_R_Freq + param.Interior_R_Phase_mult_TAU)));
		outputColor[index + 1] += color_pow_2_SSE2_FP32(param.Interior_G_Amp_mult_Interior_Alpha * (0.5f - 0.5f * cosf(logf(low) * param.Interior_G_Freq + param.Interior_G_Phase_mult_TAU)));
		outputColor[index + 2] += color_pow_2_SSE2_FP32(param.Interior_B_Amp_mult_Interior_Alpha * (0.5f - 0.5f * cosf(logf(low) * param.Interior_B_Freq + param.Interior_B_Phase_mult_TAU)));
		outputColor[index + 3] += color_pow_2_SSE2_FP32(param.Interior_Alpha);
	}
	static void CPU_Exterior_Coloring_SSE2_FP32(
		...
		outputColor[index + 0] += color_pow_2_SSE2_FP32(param.Exterior_R_Amp_mult_Exterior_Alpha * (0.5f - 0.5f * cosf(param.Exterior_R_Freq_mult_TAU * smooth + param.Exterior_R_Phase_mult_TAU)));
		outputColor[index + 1] += color_pow_2_SSE2_FP32(param.Exterior_G_Amp_mult_Exterior_Alpha * (0.5f - 0.5f * cosf(param.Exterior_G_Freq_mult_TAU * smooth + param.Exterior_G_Phase_mult_TAU)));
		outputColor[index + 2] += color_pow_2_SSE2_FP32(param.Exterior_B_Amp_mult_Exterior_Alpha * (0.5f - 0.5f * cosf(param.Exterior_B_Freq_mult_TAU * smooth + param.Exterior_B_Phase_mult_TAU)));
		outputColor[index + 3] += color_pow_2_SSE2_FP32(param.Exterior_Alpha);
	}
...
/* Fractal Rendering SSE2 FP64 */
	...
	void quadraticRender_SSE2_FP64(FractalParameters(fp64, fp64)) {
		...
		Block_Init_SSE2_FP64();
			...
			const __m128d zr1_mask = (f[3]) ? _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0x7FFFFFFFFFFFFFFF)) : _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0xFFFFFFFFFFFFFFFF));
			const __m128d zi1_mask = (f[4]) ? _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0x7FFFFFFFFFFFFFFF)) : _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0xFFFFFFFFFFFFFFFF));
			const __m128d zr2_mask = (f[5]) ? _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0x7FFFFFFFFFFFFFFF)) : _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0xFFFFFFFFFFFFFFFF));
			const __m128d zi2_mask = (f[6]) ? _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0x7FFFFFFFFFFFFFFF)) : _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0xFFFFFFFFFFFFFFFF));
			const __m128d zr_mask  = (f[7]) ? _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0x7FFFFFFFFFFFFFFF)) : _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0xFFFFFFFFFFFFFFFF));

		Block_BeginLoop_SSE2_FP64();
		
			// Applies fabs() to zr and zi
			zr1 = _mm_and_pd(zr1_mask, zr);
			zr2 = _mm_and_pd(zr2_mask, zr);
			zi1 = _mm_and_pd(zi1_mask, zi);
			zi2 = _mm_and_pd(zi2_mask, zi);
			
			// Calculates the new zr and zi
			zr = _mm_add_pd(_mm_and_pd(zr_mask, _mm_mul_pd(s1,
				_mm_sub_pd(
					_mm_mul_pd(zr1, zr), 
					_mm_mul_pd(s2,
						_mm_mul_pd(zi1, zi)
	...
	void cubicRender_SSE2_FP64(FractalParameters(fp64, fp64)) {
		...
		Block_Init_SSE2_FP64();
			...
			const __m128d zr1_mask = (f[ 6]) ? _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0x7FFFFFFFFFFFFFFF)) : _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0xFFFFFFFFFFFFFFFF));
			const __m128d zi1_mask = (f[ 7]) ? _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0x7FFFFFFFFFFFFFFF)) : _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0xFFFFFFFFFFFFFFFF));
			const __m128d zr2_mask = (f[ 8]) ? _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0x7FFFFFFFFFFFFFFF)) : _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0xFFFFFFFFFFFFFFFF));
			const __m128d zi2_mask = (f[ 9]) ? _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0x7FFFFFFFFFFFFFFF)) : _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0xFFFFFFFFFFFFFFFF));
			const __m128d zr3_mask = (f[10]) ? _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0x7FFFFFFFFFFFFFFF)) : _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0xFFFFFFFFFFFFFFFF));
			const __m128d zi3_mask = (f[11]) ? _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0x7FFFFFFFFFFFFFFF)) : _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0xFFFFFFFFFFFFFFFF));
			const __m128d zr_mask  = (f[12]) ? _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0x7FFFFFFFFFFFFFFF)) : _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0xFFFFFFFFFFFFFFFF));
			const __m128d zi_mask  = (f[13]) ? _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0x7FFFFFFFFFFFFFFF)) : _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0xFFFFFFFFFFFFFFFF));
		...
		Block_BeginLoop_SSE2_FP64();
			...
			zr2 = _mm_and_pd(zr2_mask, zr);
			zi2 = _mm_and_pd(zi2_mask, zi);
			zr3 = _mm_and_pd(zr3_mask, zr);
			zi3 = _mm_and_pd(zi3_mask, zi);

			temp_zr = _mm_add_pd(_mm_mul_pd(s5, _mm_and_pd(zr_mask,
				_mm_sub_pd(
					_mm_mul_pd(_mm_mul_pd(s1, zr1), _mm_mul_pd(zr , zr)),
					_mm_mul_pd(_mm_mul_pd(s2, zr2), _mm_mul_pd(zi1, zi))
				)
			)), cr);
			zi      = _mm_add_pd(_mm_mul_pd(s6, _mm_and_pd(zi_mask,
				_mm_sub_pd(
	...
	void quarticRender_SSE2_FP64(FractalParameters(fp64, fp64)) {
		...
		Block_Init_SSE2_FP64();
			...
			const __m128d zr1_mask = (f[ 7]) ? _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0x7FFFFFFFFFFFFFFF)) : _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0xFFFFFFFFFFFFFFFF));
			const __m128d zi1_mask = (f[ 8]) ? _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0x7FFFFFFFFFFFFFFF)) : _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0xFFFFFFFFFFFFFFFF));
			const __m128d zr2_mask = (f[ 9]) ? _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0x7FFFFFFFFFFFFFFF)) : _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0xFFFFFFFFFFFFFFFF));
			const __m128d zi2_mask = (f[10]) ? _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0x7FFFFFFFFFFFFFFF)) : _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0xFFFFFFFFFFFFFFFF));
			const __m128d zr3_mask = (f[11]) ? _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0x7FFFFFFFFFFFFFFF)) : _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0xFFFFFFFFFFFFFFFF));
			const __m128d zi3_mask = (f[12]) ? _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0x7FFFFFFFFFFFFFFF)) : _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0xFFFFFFFFFFFFFFFF));
			const __m128d zr4_mask = (f[13]) ? _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0x7FFFFFFFFFFFFFFF)) : _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0xFFFFFFFFFFFFFFFF));
			const __m128d zi4_mask = (f[14]) ? _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0x7FFFFFFFFFFFFFFF)) : _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0xFFFFFFFFFFFFFFFF));
			const __m128d zr_mask  = (f[15]) ? _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0x7FFFFFFFFFFFFFFF)) : _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0xFFFFFFFFFFFFFFFF));
			const __m128d zi_mask  = (f[16]) ? _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0x7FFFFFFFFFFFFFFF)) : _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0xFFFFFFFFFFFFFFFF));
		...
		Block_BeginLoop_SSE2_FP64();
			...
			zr3 = _mm_and_pd(zr3_mask, zr);
			zi3 = _mm_and_pd(zi3_mask, zi);
			zr4 = _mm_and_pd(zr4_mask, zr);
			zi4 = _mm_and_pd(zi4_mask, zi);

			temp_zr = _mm_add_pd(_mm_mul_pd(s6, _mm_and_pd(zr_mask,
				_mm_add_pd(
					_mm_sub_pd(
						_mm_mul_pd(s1,
			...
			zi      = _mm_add_pd(_mm_mul_pd(s7, _mm_and_pd(zi_mask,
				_mm_sub_pd(
					_mm_mul_pd(s4,
	...
	void quinticRender_SSE2_FP64(FractalParameters(fp64, fp64)) {
		...
		Block_Init_SSE2_FP64();
			...
			const __m128d zr1_mask = (fA[0]) ? _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0x7FFFFFFFFFFFFFFF)) : _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0xFFFFFFFFFFFFFFFF));
			const __m128d zi1_mask = (fA[1]) ? _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0x7FFFFFFFFFFFFFFF)) : _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0xFFFFFFFFFFFFFFFF));
			const __m128d zr2_mask = (fA[2]) ? _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0x7FFFFFFFFFFFFFFF)) : _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0xFFFFFFFFFFFFFFFF));
			const __m128d zi2_mask = (fA[3]) ? _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0x7FFFFFFFFFFFFFFF)) : _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0xFFFFFFFFFFFFFFFF));
			const __m128d zr3_mask = (fA[4]) ? _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0x7FFFFFFFFFFFFFFF)) : _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0xFFFFFFFFFFFFFFFF));
			const __m128d zi3_mask = (fA[5]) ? _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0x7FFFFFFFFFFFFFFF)) : _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0xFFFFFFFFFFFFFFFF));
			const __m128d zr4_mask = (fA[6]) ? _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0x7FFFFFFFFFFFFFFF)) : _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0xFFFFFFFFFFFFFFFF));
			const __m128d zi4_mask = (fA[7]) ? _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0x7FFFFFFFFFFFFFFF)) : _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0xFFFFFFFFFFFFFFFF));
			const __m128d zr5_mask = (fA[8]) ? _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0x7FFFFFFFFFFFFFFF)) : _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0xFFFFFFFFFFFFFFFF));
			const __m128d zi5_mask = (fA[9]) ? _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0x7FFFFFFFFFFFFFFF)) : _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0xFFFFFFFFFFFFFFFF));
			const __m128d zr_mask  = (fO[2]) ? _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0x7FFFFFFFFFFFFFFF)) : _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0xFFFFFFFFFFFFFFFF));
			const __m128d zi_mask  = (fO[3]) ? _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0x7FFFFFFFFFFFFFFF)) : _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0xFFFFFFFFFFFFFFFF));
		...
		Block_BeginLoop_SSE2_FP64();
			...
			zr4 = _mm_and_pd(zr4_mask, zr);
			zi4 = _mm_and_pd(zi4_mask, zi);
			zr5 = _mm_and_pd(zr5_mask, zr);
			zi5 = _mm_and_pd(zi5_mask, zi);

			temp_zr = _mm_add_pd(_mm_mul_pd(s7, _mm_and_pd(zr_mask ,
				_mm_add_pd(
					_mm_sub_pd(
						_mm_mul_pd(
							_mm_mul_pd(s1, zr1),
			...
			zi      = _mm_add_pd(_mm_mul_pd(s8, _mm_and_pd(zi_mask,
				_mm_add_pd(
					_mm_sub_pd(
						_mm_mul_pd(
	...
	void sexticRender_SSE2_FP64(FractalParameters(fp64, fp64)) {
		...
		Block_Init_SSE2_FP64();
			...
			const __m128d zr5_mask = (fA[ 8]) ? _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0x7FFFFFFFFFFFFFFF)) : _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0xFFFFFFFFFFFFFFFF));
			const __m128d zi5_mask = (fA[ 9]) ? _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0x7FFFFFFFFFFFFFFF)) : _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0xFFFFFFFFFFFFFFFF));
			const __m128d zr6_mask = (fA[10]) ? _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0x7FFFFFFFFFFFFFFF)) : _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0xFFFFFFFFFFFFFFFF));
			const __m128d zi6_mask = (fA[11]) ? _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0x7FFFFFFFFFFFFFFF)) : _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0xFFFFFFFFFFFFFFFF));
			const __m128d zr_mask  = (fO[ 2]) ? _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0x7FFFFFFFFFFFFFFF)) : _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0xFFFFFFFFFFFFFFFF));
			const __m128d zi_mask  = (fO[ 3]) ? _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0x7FFFFFFFFFFFFFFF)) : _mm_castsi128_pd(_mm_set1_epi64x((int64_t)0xFFFFFFFFFFFFFFFF));
		...
		Block_BeginLoop_SSE2_FP64();
			...
			zr5 = _mm_and_pd(zr5_mask, zr);
			zi5 = _mm_and_pd(zi5_mask, zi);
			zr6 = _mm_and_pd(zr6_mask, zr);
			zi6 = _mm_and_pd(zi6_mask, zi);

			temp_zr = _mm_add_pd(_mm_mul_pd(s8, _mm_and_pd(zr_mask ,
				_mm_sub_pd(
					_mm_add_pd(
						_mm_sub_pd(
							_mm_mul_pd(
			...
			zi      = _mm_add_pd(_mm_mul_pd(s9, _mm_and_pd(zi_mask,
				_mm_add_pd(
					_mm_sub_pd(
						_mm_mul_pd(
...
/* Fractal Rendering SSE2 FP32 */
	...
	void quadraticRender_SSE2_FP32(FractalParameters(fp32, fp32)) {
		...
		// Bits 3-7 will apply fabs() via a mask
		const __m128 zr1_mask = (f[3]) ? _mm_castsi128_ps(_mm_set1_epi32((int32_t)0x7FFFFFFF)) : _mm_castsi128_ps(_mm_set1_epi32((int32_t)0xFFFFFFFF));
		const __m128 zi1_mask = (f[4]) ? _mm_castsi128_ps(_mm_set1_epi32((int32_t)0x7FFFFFFF)) : _mm_castsi128_ps(_mm_set1_epi32((int32_t)0xFFFFFFFF));
		const __m128 zr2_mask = (f[5]) ? _mm_castsi128_ps(_mm_set1_epi32((int32_t)0x7FFFFFFF)) : _mm_castsi128_ps(_mm_set1_epi32((int32_t)0xFFFFFFFF));
		const __m128 zi2_mask = (f[6]) ? _mm_castsi128_ps(_mm_set1_epi32((int32_t)0x7FFFFFFF)) : _mm_castsi128_ps(_mm_set1_epi32((int32_t)0xFFFFFFFF));
		const __m128 zr_mask  = (f[7]) ? _mm_castsi128_ps(_mm_set1_epi32((int32_t)0x7FFFFFFF)) : _mm_castsi128_ps(_mm_set1_epi32((int32_t)0xFFFFFFFF));
		...
		Block_BeginLoop_SSE2_FP32();
			...
			// Applies fabs() to zr and zi
			zr1 = _mm_and_ps(zr1_mask, zr);
			zr2 = _mm_and_ps(zr2_mask, zr);
			zi1 = _mm_and_ps(zi1_mask, zi);
			zi2 = _mm_and_ps(zi2_mask, zi);
			
			// Calculates the new zr and zi
			zr = _mm_add_ps(_mm_and_ps(zr_mask, _mm_mul_ps(s1,
				_mm_sub_ps(
					_mm_mul_ps(zr1, zr),
					_mm_mul_ps(s2,
						_mm_mul_ps(zi1, zi)
	...
	void cubicRender_SSE2_FP32(FractalParameters(fp32, fp32)) {
		...
		Block_Init_SSE2_FP32();
			...
			const __m128 zr1_mask = (f[ 6]) ? _mm_castsi128_ps(_mm_set1_epi32((int32_t)0x7FFFFFFF)) : _mm_castsi128_ps(_mm_set1_epi32((int32_t)0xFFFFFFFF));
			const __m128 zi1_mask = (f[ 7]) ? _mm_castsi128_ps(_mm_set1_epi32((int32_t)0x7FFFFFFF)) : _mm_castsi128_ps(_mm_set1_epi32((int32_t)0xFFFFFFFF));
			const __m128 zr2_mask = (f[ 8]) ? _mm_castsi128_ps(_mm_set1_epi32((int32_t)0x7FFFFFFF)) : _mm_castsi128_ps(_mm_set1_epi32((int32_t)0xFFFFFFFF));
			const __m128 zi2_mask = (f[ 9]) ? _mm_castsi128_ps(_mm_set1_epi32((int32_t)0x7FFFFFFF)) : _mm_castsi128_ps(_mm_set1_epi32((int32_t)0xFFFFFFFF));
			const __m128 zr3_mask = (f[10]) ? _mm_castsi128_ps(_mm_set1_epi32((int32_t)0x7FFFFFFF)) : _mm_castsi128_ps(_mm_set1_epi32((int32_t)0xFFFFFFFF));
			const __m128 zi3_mask = (f[11]) ? _mm_castsi128_ps(_mm_set1_epi32((int32_t)0x7FFFFFFF)) : _mm_castsi128_ps(_mm_set1_epi32((int32_t)0xFFFFFFFF));
			const __m128 zr_mask  = (f[12]) ? _mm_castsi128_ps(_mm_set1_epi32((int32_t)0x7FFFFFFF)) : _mm_castsi128_ps(_mm_set1_epi32((int32_t)0xFFFFFFFF));
			const __m128 zi_mask  = (f[13]) ? _mm_castsi128_ps(_mm_set1_epi32((int32_t)0x7FFFFFFF)) : _mm_castsi128_ps(_mm_set1_epi32((int32_t)0xFFFFFFFF));
		...
		Block_BeginLoop_SSE2_FP32();
			...
			zr2 = _mm_and_ps(zr2_mask, zr);
			zi2 = _mm_and_ps(zi2_mask, zi);
			zr3 = _mm_and_ps(zr3_mask, zr);
			zi3 = _mm_and_ps(zi3_mask, zi);

			temp_zr = _mm_add_ps(_mm_mul_ps(s5, _mm_and_ps(zr_mask,
				_mm_sub_ps(
					_mm_mul_ps(_mm_mul_ps(s1, zr1), _mm_mul_ps(zr , zr)),
					_mm_mul_ps(_mm_mul_ps(s2, zr2), _mm_mul_ps(zi1, zi))
				)
			)), cr);
			zi      = _mm_add_ps(_mm_mul_ps(s6, _mm_and_ps(zi_mask,
				_mm_sub_ps(
	...
	void quarticRender_SSE2_FP32(FractalParameters(fp32, fp32)) {
		...
		Block_Init_SSE2_FP32();
			...
			const __m128 zr1_mask = (f[ 7]) ? _mm_castsi128_ps(_mm_set1_epi32((int32_t)0x7FFFFFFF)) : _mm_castsi128_ps(_mm_set1_epi32((int32_t)0xFFFFFFFF));
			const __m128 zi1_mask = (f[ 8]) ? _mm_castsi128_ps(_mm_set1_epi32((int32_t)0x7FFFFFFF)) : _mm_castsi128_ps(_mm_set1_epi32((int32_t)0xFFFFFFFF));
			const __m128 zr2_mask = (f[ 9]) ? _mm_castsi128_ps(_mm_set1_epi32((int32_t)0x7FFFFFFF)) : _mm_castsi128_ps(_mm_set1_epi32((int32_t)0xFFFFFFFF));
			const __m128 zi2_mask = (f[10]) ? _mm_castsi128_ps(_mm_set1_epi32((int32_t)0x7FFFFFFF)) : _mm_castsi128_ps(_mm_set1_epi32((int32_t)0xFFFFFFFF));
			const __m128 zr3_mask = (f[11]) ? _mm_castsi128_ps(_mm_set1_epi32((int32_t)0x7FFFFFFF)) : _mm_castsi128_ps(_mm_set1_epi32((int32_t)0xFFFFFFFF));
			const __m128 zi3_mask = (f[12]) ? _mm_castsi128_ps(_mm_set1_epi32((int32_t)0x7FFFFFFF)) : _mm_castsi128_ps(_mm_set1_epi32((int32_t)0xFFFFFFFF));
			const __m128 zr4_mask = (f[13]) ? _mm_castsi128_ps(_mm_set1_epi32((int32_t)0x7FFFFFFF)) : _mm_castsi128_ps(_mm_set1_epi32((int32_t)0xFFFFFFFF));
			const __m128 zi4_mask = (f[14]) ? _mm_castsi128_ps(_mm_set1_epi32((int32_t)0x7FFFFFFF)) : _mm_castsi128_ps(_mm_set1_epi32((int32_t)0xFFFFFFFF));
			const __m128 zr_mask  = (f[15]) ? _mm_castsi128_ps(_mm_set1_epi32((int32_t)0x7FFFFFFF)) : _mm_castsi128_ps(_mm_set1_epi32((int32_t)0xFFFFFFFF));
			const __m128 zi_mask  = (f[16]) ? _mm_castsi128_ps(_mm_set1_epi32((int32_t)0x7FFFFFFF)) : _mm_castsi128_ps(_mm_set1_epi32((int32_t)0xFFFFFFFF));
		...
		Block_BeginLoop_SSE2_FP32();
			...
			zr3 = _mm_and_ps(zr3_mask, zr);
			zi3 = _mm_and_ps(zi3_mask, zi);
			zr4 = _mm_and_ps(zr4_mask, zr);
			zi4 = _mm_and_ps(zi4_mask, zi);

			temp_zr = _mm_add_ps(_mm_mul_ps(s6, _mm_and_ps(zr_mask ,
				_mm_add_ps(
					_mm_sub_ps(
						_mm_mul_ps(s1,
			...
			zi      = _mm_add_ps(_mm_mul_ps(s7, _mm_and_ps(zi_mask,
				_mm_sub_ps(
					_mm_mul_ps(s4,
	...
	void quinticRender_SSE2_FP32(FractalParameters(fp32, fp32)) {
		...
		Block_Init_SSE2_FP32();
			...
			const __m128 zr1_mask = (fA[0]) ? _mm_castsi128_ps(_mm_set1_epi32((int32_t)0x7FFFFFFF)) : _mm_castsi128_ps(_mm_set1_epi32((int32_t)0xFFFFFFFF));
			const __m128 zi1_mask = (fA[1]) ? _mm_castsi128_ps(_mm_set1_epi32((int32_t)0x7FFFFFFF)) : _mm_castsi128_ps(_mm_set1_epi32((int32_t)0xFFFFFFFF));
			const __m128 zr2_mask = (fA[2]) ? _mm_castsi128_ps(_mm_set1_epi32((int32_t)0x7FFFFFFF)) : _mm_castsi128_ps(_mm_set1_epi32((int32_t)0xFFFFFFFF));
			const __m128 zi2_mask = (fA[3]) ? _mm_castsi128_ps(_mm_set1_epi32((int32_t)0x7FFFFFFF)) : _mm_castsi128_ps(_mm_set1_epi32((int32_t)0xFFFFFFFF));
			const __m128 zr3_mask = (fA[4]) ? _mm_castsi128_ps(_mm_set1_epi32((int32_t)0x7FFFFFFF)) : _mm_castsi128_ps(_mm_set1_epi32((int32_t)0xFFFFFFFF));
			const __m128 zi3_mask = (fA[5]) ? _mm_castsi128_ps(_mm_set1_epi32((int32_t)0x7FFFFFFF)) : _mm_castsi128_ps(_mm_set1_epi32((int32_t)0xFFFFFFFF));
			const __m128 zr4_mask = (fA[6]) ? _mm_castsi128_ps(_mm_set1_epi32((int32_t)0x7FFFFFFF)) : _mm_castsi128_ps(_mm_set1_epi32((int32_t)0xFFFFFFFF));
			const __m128 zi4_mask = (fA[7]) ? _mm_castsi128_ps(_mm_set1_epi32((int32_t)0x7FFFFFFF)) : _mm_castsi128_ps(_mm_set1_epi32((int32_t)0xFFFFFFFF));
			const __m128 zr5_mask = (fA[8]) ? _mm_castsi128_ps(_mm_set1_epi32((int32_t)0x7FFFFFFF)) : _mm_castsi128_ps(_mm_set1_epi32((int32_t)0xFFFFFFFF));
			const __m128 zi5_mask = (fA[9]) ? _mm_castsi128_ps(_mm_set1_epi32((int32_t)0x7FFFFFFF)) : _mm_castsi128_ps(_mm_set1_epi32((int32_t)0xFFFFFFFF));
			const __m128 zr_mask  = (fO[2]) ? _mm_castsi128_ps(_mm_set1_epi32((int32_t)0x7FFFFFFF)) : _mm_castsi128_ps(_mm_set1_epi32((int32_t)0xFFFFFFFF));
			const __m128 zi_mask  = (fO[3]) ? _mm_castsi128_ps(_mm_set1_epi32((int32_t)0x7FFFFFFF)) : _mm_castsi128_ps(_mm_set1_epi32((int32_t)0xFFFFFFFF));
		...
		Block_BeginLoop_SSE2_FP32();
			...
			zr4 = _mm_and_ps(zr4_mask, zr);
			zi4 = _mm_and_ps(zi4_mask, zi);
			zr5 = _mm_and_ps(zr5_mask, zr);
			zi5 = _mm_and_ps(zi5_mask, zi);

			temp_zr = _mm_add_ps(_mm_mul_ps(s7, _mm_and_ps(zr_mask ,
				_mm_add_ps(
					_mm_sub_ps(
						_mm_mul_ps(
							_mm_mul_ps(s1, zr1),
			...
			zi      = _mm_add_ps(_mm_mul_ps(s8, _mm_and_ps(zi_mask,
				_mm_add_ps(
					_mm_sub_ps(
						_mm_mul_ps(
	...
	void sexticRender_SSE2_FP32(FractalParameters(fp32, fp32)) {
		...
		Block_Init_SSE2_FP32();
			...
			const __m128 zr5_mask = (fA[ 8]) ? _mm_castsi128_ps(_mm_set1_epi32((int32_t)0x7FFFFFFF)) : _mm_castsi128_ps(_mm_set1_epi32((int32_t)0xFFFFFFFF));
			const __m128 zi5_mask = (fA[ 9]) ? _mm_castsi128_ps(_mm_set1_epi32((int32_t)0x7FFFFFFF)) : _mm_castsi128_ps(_mm_set1_epi32((int32_t)0xFFFFFFFF));
			const __m128 zr6_mask = (fA[10]) ? _mm_castsi128_ps(_mm_set1_epi32((int32_t)0x7FFFFFFF)) : _mm_castsi128_ps(_mm_set1_epi32((int32_t)0xFFFFFFFF));
			const __m128 zi6_mask = (fA[11]) ? _mm_castsi128_ps(_mm_set1_epi32((int32_t)0x7FFFFFFF)) : _mm_castsi128_ps(_mm_set1_epi32((int32_t)0xFFFFFFFF));
			const __m128 zr_mask  = (fO[ 2]) ? _mm_castsi128_ps(_mm_set1_epi32((int32_t)0x7FFFFFFF)) : _mm_castsi128_ps(_mm_set1_epi32((int32_t)0xFFFFFFFF));
			const __m128 zi_mask  = (fO[ 3]) ? _mm_castsi128_ps(_mm_set1_epi32((int32_t)0x7FFFFFFF)) : _mm_castsi128_ps(_mm_set1_epi32((int32_t)0xFFFFFFFF));
		...
		Block_BeginLoop_SSE2_FP32();
			...
			zr5 = _mm_and_ps(zr5_mask, zr);
			zi5 = _mm_and_ps(zi5_mask, zi);
			zr6 = _mm_and_ps(zr6_mask, zr);
			zi6 = _mm_and_ps(zi6_mask, zi);
			#define _M_ADD(x,y) (x + y)
			#define _M_SUB(x,y) (x - y)
			#define _M_MUL(x,y) (x * y)
			temp_zr = _mm_add_ps(_mm_mul_ps(s8, _mm_and_ps(zr_mask ,
				_mm_sub_ps(
					_mm_add_ps(
						_mm_sub_ps(
							_mm_mul_ps(
			...
			zi      = _mm_add_ps(_mm_mul_ps(s9, _mm_and_ps(zi_mask,
				_mm_add_ps(
					_mm_sub_ps(
						_mm_mul_ps(
