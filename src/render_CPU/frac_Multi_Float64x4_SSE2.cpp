...
/* 
	Directory:
	*	Debug Tools
	*	Function Definition SSE2 FP64x4
	*	Fractal Rendering SSE2 FP64x4
*/
...
/* Function Definition SSE2 FP64 */
	...
	static inline __m128dx4 _mm128x4_mask_fabs_pdx4(__m128dx4 x, __m128d mask) {
		const __m128d sign_mask = _internal_mm_blendv_pd(
			_mm_set1_pd( 1.0),
			_mm_set1_pd(-1.0),
			_mm_and_pd(x.val[0], mask)
		);
		x.val[0] = _mm_mul_pd(x.val[0], sign_mask);
		x.val[1] = _mm_mul_pd(x.val[1], sign_mask);
		x.val[2] = _mm_mul_pd(x.val[2], sign_mask);
		x.val[3] = _mm_mul_pd(x.val[3], sign_mask);
		return x;
	}

	#define color_pow_2_SSE2_FP64x4(x) ((x) * (x))

	static void CPU_Interior_Coloring_SSE2_FP64x4(
		fp64* outputColor, size_t index,
		const PreCalc_Param<Float64x4, fp64>& param,
		Float64x4 low
	) {
		outputColor[index + 0] += (fp64)color_pow_2_SSE2_FP64x4(param.Interior_R_Amp_mult_Interior_Alpha * (0.5 - 0.5 * cos((fp64)log(low.val[0]) * param.Interior_R_Freq + param.Interior_R_Phase_mult_TAU)));
		outputColor[index + 1] += (fp64)color_pow_2_SSE2_FP64x4(param.Interior_G_Amp_mult_Interior_Alpha * (0.5 - 0.5 * cos((fp64)log(low.val[0]) * param.Interior_G_Freq + param.Interior_G_Phase_mult_TAU)));
		outputColor[index + 2] += (fp64)color_pow_2_SSE2_FP64x4(param.Interior_B_Amp_mult_Interior_Alpha * (0.5 - 0.5 * cos((fp64)log(low.val[0]) * param.Interior_B_Freq + param.Interior_B_Phase_mult_TAU)));
		outputColor[index + 3] += (fp64)color_pow_2_SSE2_FP64x4(param.Interior_Alpha);
	}
	static void CPU_Exterior_Coloring_SSE2_FP64x4(
		fp64* outputColor, size_t index,
		const PreCalc_Param<Float64x4, fp64>& param,
		...
		outputColor[index + 0] += (fp64)color_pow_2_SSE2_FP64x4(param.Exterior_R_Amp_mult_Exterior_Alpha * (0.5 - 0.5 * cos(param.Exterior_R_Freq_mult_TAU * smooth + param.Exterior_R_Phase_mult_TAU)));
		outputColor[index + 1] += (fp64)color_pow_2_SSE2_FP64x4(param.Exterior_G_Amp_mult_Exterior_Alpha * (0.5 - 0.5 * cos(param.Exterior_G_Freq_mult_TAU * smooth + param.Exterior_G_Phase_mult_TAU)));
		outputColor[index + 2] += (fp64)color_pow_2_SSE2_FP64x4(param.Exterior_B_Amp_mult_Exterior_Alpha * (0.5 - 0.5 * cos(param.Exterior_B_Freq_mult_TAU * smooth + param.Exterior_B_Phase_mult_TAU)));
		outputColor[index + 3] += (fp64)color_pow_2_SSE2_FP64x4(param.Exterior_Alpha);
	}
	
	#define Block_Init_SSE2_FP64x4();

	#define Block_BeginLoop_SSE2_FP64x4(); \
		constexpr size_t SIMD_Spacing = 2; /* SIMD_Spacing Registers in SSE2 FP64 */\
		\
		/* Load Precalculated constants */\
		const __m128dx4 realCord       = _mm128x4_set1_pdx4(param.realCord     );\
		const __m128dx4 imagCord       = _mm128x4_set1_pdx4(param.imagCord     );\
		const __m128dx4 realJulia      = _mm128x4_set1_pdx4(param.realJulia    );\
		const __m128dx4 imagJulia      = _mm128x4_set1_pdx4(param.imagJulia    );\
		const __m128dx4 rotSin_PC      = _mm128x4_set1_pdx4(param.rotSin_PC    );\
		const __m128dx4 rotCos_PC      = _mm128x4_set1_pdx4(param.rotCos_PC    );\
		const __m128dx4 breakoutValue  = _mm128x4_set1_pdx4(param.breakoutValue);\
		...
		const __m128dx4 recip_numZ     = _mm128x4_set1_pdx4(param.recip_numZ    );\
		const __m128dx4 neg_recip_numW = _mm128x4_set1_pdx4(param.neg_recip_numW);\
		\
		/* Init Loop */\
		...
		for (; y < param.Cord_ResY; y += param.sample) {\
			for (; x < param.Cord_ResX; x += (int32_t)SIMD_Spacing * param.sample) {\
				...
				for (int32_t v = 0; v < param.sample; v++) {\
					...
					__m128dx4 yCord = _mm128x4_sub_pd_pd(_mm_set1_pd((fp64)y), numY);\
					yCord = _mm128x4_mul_pdx4(yCord, neg_recip_numW);\
					\
					for (int32_t u = 0; u < param.sample; u++) {\
						/* Calculates the SIMD_Spacing x cordinate-values */\
						__m128dx4 xCord = _mm128x4_sub_pd_pd(\
							_mm_set_pd(\
						...
						xCord = _mm128x4_mul_pdx4(xCord, recip_numZ);\
						\
						/* Transforms the x and y cordinate-values into the cordinates */\
						/* `cr` and `ci` are swapped with `zr` and `zi` if `param.juliaSet == true` */\
						__m128dx4 cr = (!param.juliaSet) ? _mm128x4_add_pdx4(_mm128x4_sub_pdx4(_mm128x4_mul_pdx4(xCord, rotCos_PC), _mm128x4_mul_pdx4(yCord, rotSin_PC)), realCord) : realJulia;\
						__m128dx4 ci = (!param.juliaSet) ? _mm128x4_add_pdx4(_mm128x4_add_pdx4(_mm128x4_mul_pdx4(yCord, rotCos_PC), _mm128x4_mul_pdx4(xCord, rotSin_PC)), imagCord) : imagJulia;\
						__m128dx4 zr = ( param.juliaSet) ? _mm128x4_add_pdx4(_mm128x4_sub_pdx4(_mm128x4_mul_pdx4(xCord, rotCos_PC), _mm128x4_mul_pdx4(yCord, rotSin_PC)), realCord) : realJulia;\
						__m128dx4 zi = ( param.juliaSet) ? _mm128x4_add_pdx4(_mm128x4_add_pdx4(_mm128x4_mul_pdx4(yCord, rotCos_PC), _mm128x4_mul_pdx4(xCord, rotSin_PC)), imagCord) : imagJulia;\
						\
						__m128dx4 low = _mm128x4_set1_pd(4.0);\
						__m128dx4 zs = _mm128x4_setzero_pdx4();\
						__m128dx4 __attribute__((unused)) temp_zr = _mm128x4_setzero_pdx4();\
						__m128dx4 current_value_mask = _mm128x4_set1_epi64x((long long)0xFFFFFFFFFFFFFFFF);\
	...
	#define Block_EndLoop_SSE2_FP64x4(); \
							...
							if (break_mask != 0) {\
								...
								for (int i = 0; i < (int)SIMD_Spacing; i++) {\
									if (break_mask & (1 << i)) {\
										...
										Float64x4 zs_val;\
										zs_val.val[0] = zs_arr[i + 0 * (int)SIMD_Spacing];\
										zs_val.val[1] = zs_arr[i + 1 * (int)SIMD_Spacing];\
										zs_val.val[2] = zs_arr[i + 2 * (int)SIMD_Spacing];\
										zs_val.val[3] = zs_arr[i + 3 * (int)SIMD_Spacing];\
										CPU_Exterior_Coloring_SSE2_FP64x4(\
										...
										cvmask_arr[i + 0 * (int)SIMD_Spacing] = 0.0;\
										cvmask_arr[i + 1 * (int)SIMD_Spacing] = 0.0;\
										cvmask_arr[i + 2 * (int)SIMD_Spacing] = 0.0;\
										cvmask_arr[i + 3 * (int)SIMD_Spacing] = 0.0;\
									}\
								}\
								if (_mm_movemask_pd(_mm_cmpeq_pdx4(current_value_mask, _mm128x4_setzero_pdx4())) == 0) {\
						...
						int inside_value_mask = _mm_movemask_pd(_mm_cmpneq_pdx4(current_value_mask, _mm128x4_setzero_pdx4()));\
						if (inside_value_mask != 0) {\
							...
							for (int i = 0; i < (int)SIMD_Spacing; i++) {\
								if (inside_value_mask & (1 << i)) {\
									...
									Float64x4 low_val;\
									low_val.val[0] = low_arr[i + 0 * (int)SIMD_Spacing];\
									low_val.val[1] = low_arr[i + 1 * (int)SIMD_Spacing];\
									low_val.val[2] = low_arr[i + 2 * (int)SIMD_Spacing];\
									low_val.val[3] = low_arr[i + 3 * (int)SIMD_Spacing];\
									CPU_Interior_Coloring_SSE2_FP64x4(\
										outputColor, (size_t)i * IMAGE_BUFFER_CHANNELS,\
										param,\
										low_val\
									);\
...
/* Fractal Rendering SSE2 FP64 */
	...
	void quadraticRender_SSE2_FP64x4(FractalParameters(Float64x4, fp64)) {
		...
		Block_Init_SSE2_FP64x4();
			__m128dx4 zr1, zr2, zi1, zi2;
		...
		Block_BeginLoop_SSE2_FP64x4();
			
			// Applies fabs() to zr and zi
			zr1 = _mm128x4_mask_fabs_pdx4(zr, zr1_mask);
			zr2 = _mm128x4_mask_fabs_pdx4(zr, zr2_mask);
			zi1 = _mm128x4_mask_fabs_pdx4(zi, zi1_mask);
			zi2 = _mm128x4_mask_fabs_pdx4(zi, zi2_mask);
			
			// Calculates the new zr and zi
			zr = _mm128x4_add_pdx4(_mm128x4_mask_fabs_pdx4(_mm128x4_mul_pd_pdx4(s1,
				_mm128x4_sub_pdx4(
					_mm128x4_mul_pdx4(zr1, zr), 
					_mm128x4_mul_pd_pdx4(s2,
						_mm128x4_mul_pdx4(zi1, zi)
					)
				)
			), zr_mask), cr);
			zi = _mm128x4_add_pdx4(_mm128x4_mul_pdx4_pd(_mm128x4_mul_pdx4(zr2, zi2), s3), ci);

			zs = _mm128x4_add_pdx4(_mm128x4_mul_pdx4(zr, zr), _mm128x4_mul_pdx4(zi, zi));
			
		
		Block_EndLoop_SSE2_FP64x4();
	}
	...
	void cubicRender_SSE2_FP64x4(FractalParameters(Float64x4, fp64)) {
		...
		Block_Init_SSE2_FP64x4();
			...
			__m128dx4 zr1, zr2, zr3, zi1, zi2, zi3;
			bool f[14];
		...
		Block_BeginLoop_SSE2_FP64x4();
			...
			zr2 = _mm128x4_mask_fabs_pdx4(zr, zr2_mask);
			zi2 = _mm128x4_mask_fabs_pdx4(zi, zi2_mask);
			zr3 = _mm128x4_mask_fabs_pdx4(zr, zr3_mask);
			zi3 = _mm128x4_mask_fabs_pdx4(zi, zi3_mask);

			temp_zr = _mm128x4_add_pdx4(_mm128x4_mul_pd_pdx4(s5, _mm128x4_mask_fabs_pdx4(
				_mm128x4_sub_pdx4(
					_mm128x4_mul_pdx4(_mm128x4_mul_pd_pdx4(s1, zr1), _mm128x4_mul_pdx4(zr , zr)),
					_mm128x4_mul_pdx4(_mm128x4_mul_pd_pdx4(s2, zr2), _mm128x4_mul_pdx4(zi1, zi))
				),
			zr_mask)), cr);
			zi      = _mm128x4_add_pdx4(_mm128x4_mul_pd_pdx4(s6, _mm128x4_mask_fabs_pdx4(
				_mm128x4_sub_pdx4(
					_mm128x4_mul_pdx4(_mm128x4_mul_pd_pdx4(s3, zr3), _mm128x4_mul_pdx4(zr, zi2)),
					_mm128x4_mul_pdx4(_mm128x4_mul_pd_pdx4(s4, zi3), _mm128x4_mul_pdx4(zi, zi ))
				),
			zi_mask)), ci);
			zr = temp_zr;
	...
	void quarticRender_SSE2_FP64x4(FractalParameters(Float64x4, fp64)) {
		...
		Block_Init_SSE2_FP64x4();
			...
			__m128dx4 zr1, zr2, zr3, zr4, zi1, zi2, zi3, zi4;
			bool f[17];
		...
		Block_BeginLoop_SSE2_FP64x4();
			...
			zr3 = _mm128x4_mask_fabs_pdx4(zr, zr3_mask);
			zi3 = _mm128x4_mask_fabs_pdx4(zi, zi3_mask);
			zr4 = _mm128x4_mask_fabs_pdx4(zr, zr4_mask);
			zi4 = _mm128x4_mask_fabs_pdx4(zi, zi4_mask);

			temp_zr = _mm128x4_add_pdx4(_mm128x4_mul_pd_pdx4(s6, _mm128x4_mask_fabs_pdx4(
				_mm128x4_add_pdx4(
					_mm128x4_sub_pdx4(
						_mm128x4_mul_pd_pdx4(s1,
							_mm128x4_mul_pdx4(_mm128x4_mul_pdx4(zr1, zr ), _mm128x4_mul_pdx4(zr, zr))
						),
						_mm128x4_mul_pd_pdx4(s2,
							_mm128x4_mul_pdx4(_mm128x4_mul_pdx4(zr2, zi1), _mm128x4_mul_pdx4(zr, zi))
						)
					),
					_mm128x4_mul_pd_pdx4(s3,
						_mm128x4_mul_pdx4(_mm128x4_mul_pdx4(zi2, zi), _mm128x4_mul_pdx4(zi, zi))
					)
				),
			zr_mask)), cr);
			zi      = _mm128x4_add_pdx4(_mm128x4_mul_pd_pdx4(s7, _mm128x4_mask_fabs_pdx4(
				_mm128x4_sub_pdx4(
					_mm128x4_mul_pd_pdx4(s4,
						_mm128x4_mul_pdx4(_mm128x4_mul_pdx4(zr3, zi3), _mm128x4_mul_pdx4(zr, zr))
					),
					_mm128x4_mul_pd_pdx4(s5,
						_mm128x4_mul_pdx4(_mm128x4_mul_pdx4(zr4, zi4), _mm128x4_mul_pdx4(zi, zi))
					)
				),
			zi_mask)), ci);
			zr = temp_zr;
	...
	void quinticRender_SSE2_FP64x4(FractalParameters(Float64x4, fp64)) {
		...
		Block_Init_SSE2_FP64x4();
			...
			__m128dx4 zr1, zr2, zr3, zr4, zr5, zi1, zi2, zi3, zi4, zi5;
			bool fS[6];
			bool fA[10];
			bool fO[4];
		...
		Block_BeginLoop_SSE2_FP64x4();
			...
			zr4 = _mm128x4_mask_fabs_pdx4(zr, zr4_mask);
			zi4 = _mm128x4_mask_fabs_pdx4(zi, zi4_mask);
			zr5 = _mm128x4_mask_fabs_pdx4(zr, zr5_mask);
			zi5 = _mm128x4_mask_fabs_pdx4(zi, zi5_mask);

			temp_zr = _mm128x4_add_pdx4(_mm128x4_mul_pd_pdx4(s7, _mm128x4_mask_fabs_pdx4(
				_mm128x4_add_pdx4(
					_mm128x4_sub_pdx4(
						_mm128x4_mul_pdx4(
							_mm128x4_mul_pd_pdx4(s1, zr1),
							_mm128x4_mul_pdx4(_mm128x4_mul_pdx4(zr, zr), _mm128x4_mul_pdx4(zr , zr))
						),
						_mm128x4_mul_pdx4(
							_mm128x4_mul_pd_pdx4(s2, _mm128x4_mul_pdx4(zr2, zi1)),
							_mm128x4_mul_pdx4(zi, _mm128x4_mul_pdx4(zr , zr ))
						)
					),
					_mm128x4_mul_pdx4(
						_mm128x4_mul_pd_pdx4(s3, _mm128x4_mul_pdx4(zr3, zi2)),
						_mm128x4_mul_pdx4(zi, _mm128x4_mul_pdx4(zi , zi ))
					)
				),
			zr_mask)), cr);
			zi      = _mm128x4_add_pdx4(_mm128x4_mul_pd_pdx4(s8, _mm128x4_mask_fabs_pdx4(
				_mm128x4_add_pdx4(
					_mm128x4_sub_pdx4(
						_mm128x4_mul_pdx4(
							_mm128x4_mul_pd_pdx4(s4 , _mm128x4_mul_pdx4(zr4, zr)),
							_mm128x4_mul_pdx4(zi3, _mm128x4_mul_pdx4(zr , zr))
						),
						_mm128x4_mul_pdx4(
							_mm128x4_mul_pd_pdx4(s5 , _mm128x4_mul_pdx4(zr5, zr)),
							_mm128x4_mul_pdx4(zi4, _mm128x4_mul_pdx4(zi , zi))
						)
					),
					_mm128x4_mul_pdx4(
						_mm128x4_mul_pd_pdx4(s6 , zi5),
						_mm128x4_mul_pdx4(_mm128x4_mul_pdx4(zi, zi), _mm128x4_mul_pdx4(zi , zi))
					)
				),
			zi_mask)), ci);
			zr = temp_zr;
	...
	void sexticRender_SSE2_FP64x4(FractalParameters(Float64x4, fp64)) {
		...
		Block_Init_SSE2_FP64x4();
			...
			__m128dx4 zr1, zr2, zr3, zr4, zr5, zr6, zi1, zi2, zi3, zi4, zi5, zi6;
			bool fS[7];
			bool fA[12];
			bool fO[4];
		...
		Block_BeginLoop_SSE2_FP64x4();
			...
			zr5 = _mm128x4_mask_fabs_pdx4(zr, zr5_mask);
			zi5 = _mm128x4_mask_fabs_pdx4(zi, zi5_mask);
			zr6 = _mm128x4_mask_fabs_pdx4(zr, zr6_mask);
			zi6 = _mm128x4_mask_fabs_pdx4(zi, zi6_mask);

			temp_zr = _mm128x4_add_pdx4(_mm128x4_mul_pd_pdx4(s8, _mm128x4_mask_fabs_pdx4(
				_mm128x4_sub_pdx4(
					_mm128x4_add_pdx4(
						_mm128x4_sub_pdx4(
							_mm128x4_mul_pdx4(
								_mm128x4_mul_pd_pdx4(s1, _mm128x4_mul_pdx4(zr1, zr)),
								_mm128x4_mul_pdx4(_mm128x4_mul_pdx4(zr, zr), _mm128x4_mul_pdx4(zr, zr))
							),
							_mm128x4_mul_pdx4(
								_mm128x4_mul_pd_pdx4(s2, _mm128x4_mul_pdx4(zr2, zi1)),
								_mm128x4_mul_pdx4(_mm128x4_mul_pdx4(zr, zr), _mm128x4_mul_pdx4(zr, zi))
							)
						),
						_mm128x4_mul_pdx4(
							_mm128x4_mul_pd_pdx4(s3, _mm128x4_mul_pdx4(zr3, zi2)),
							_mm128x4_mul_pdx4(_mm128x4_mul_pdx4(zr, zi), _mm128x4_mul_pdx4(zi, zi))
						)
					),
					_mm128x4_mul_pdx4(
						_mm128x4_mul_pd_pdx4(s4, _mm128x4_mul_pdx4(zi3, zi)),
						_mm128x4_mul_pdx4(_mm128x4_mul_pdx4(zi, zi), _mm128x4_mul_pdx4(zi, zi))
					)
				),
			zr_mask)), cr);
			zi      = _mm128x4_add_pdx4(_mm128x4_mul_pd_pdx4(s9, _mm128x4_mask_fabs_pdx4(
				_mm128x4_add_pdx4(
					_mm128x4_sub_pdx4(
						_mm128x4_mul_pdx4(
							_mm128x4_mul_pd_pdx4(s5, _mm128x4_mul_pdx4(zr4, zi4)),
							_mm128x4_mul_pdx4(_mm128x4_mul_pdx4(zr, zr), _mm128x4_mul_pdx4(zr, zr))
						),
						_mm128x4_mul_pdx4(
							_mm128x4_mul_pd_pdx4(s6, _mm128x4_mul_pdx4(zr5, zi5)),
							_mm128x4_mul_pdx4(_mm128x4_mul_pdx4(zr, zr), _mm128x4_mul_pdx4(zi, zi))
						)
					),
					_mm128x4_mul_pdx4(
						_mm128x4_mul_pd_pdx4(s7, _mm128x4_mul_pdx4(zr6, zi6)),
						_mm128x4_mul_pdx4(_mm128x4_mul_pdx4(zi, zi), _mm128x4_mul_pdx4(zi, zi))
					)
				),
			zi_mask)), ci);
