...
const KeyBox Keyboard_NULLKEY = {"Unknown",0,0,0,0,"SDLK_UNKNOWN","SDL_SCANCODE_UNKNOWN"};
...
uint32_t Scancode_Color[SDL_NUM_SCANCODES];

void renderKeyboard(BufferBox* buf, uint8_t keyboardSize, bool includeNumpad) {
	if (buf == NULL || keyboardSize >= Number_of_Keyboard_Sizes) { return; }
...
void setRGB_Scancode(uint8_t r, uint8_t g, uint8_t b, SDL_Scancode code) {
	Scancode_Color[code] = (r << 16) + (g << 8) + b;
}
